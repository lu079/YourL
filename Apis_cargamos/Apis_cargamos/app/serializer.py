...
class ProductSerializer(ma.SQLAlchemyAutoSchema):
    class Meta:
        model=Product
        fields=('id','name','presentation','description','posology','management','stock_code','category_id','price','discount_price')
...
class OrderSerializer(ma.SQLAlchemyAutoSchema):
    class Meta:
        model=Order
        fields=('id','user_id','order_num','delivery_date')
...
class UserAddressSerializer(ma.SQLAlchemyAutoSchema):
    class Meta:
        model=UserAddress
        fields=('id','user_id','address1','address2','zipcode','country','city','phone')
userAddress_schema=OrderSerializer()

class PaymentSerializer(ma.SQLAlchemyAutoSchema):
    class Meta:
        model=Payment
        fields=('id','order_id','paymant_date','payment_type')
